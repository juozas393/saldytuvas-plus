/**
 * Lidl Scraper Runner
 * Direct execution script for Lidl scraping
 */

import 'dotenv/config';
import { LidlAdapter } from './LidlAdapter.js';
...
async function main() {
    ...
    // Debug: Write full result info to debug file
    const debugInfo = {
    ...
    fs.writeFileSync('./debug_output.json', JSON.stringify(debugInfo, null, 2));
    ...
    fs.writeFileSync('./lidl_products.json', JSON.stringify(exportProducts, null, 2));
    console.log('Results saved to: lidl_products.json');
}
